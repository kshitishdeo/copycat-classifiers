#From old recognition code - written by Cheryl Wang
#Performs feature extraction from raw Kinect 2.0 data
...
featureIndex = [int(i) for i in sys.argv[3].split(",")]


inputFile = io.open(sys.argv[1], "r", encoding="utf-16-le")

name = sys.argv[1].replace(".txt", ".ark")
name = name.replace("data/txt/", "")
outputFileName = arkFolder + "/" + name.replace("/", "_") 
